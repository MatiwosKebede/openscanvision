...
object ImagePreprocessor {
    fun enhanceContrast(bitmap: Bitmap): Bitmap {
        ...
        src.release(); gray.release(); destGray.release(); dest.release()
        return result
    }
    ...
    fun denoise(bitmap: Bitmap): Bitmap {
        ...
        val result = Bitmap.createBitmap(bitmap.width, bitmap.height, Bitmap.Config.ARGB_8888)
        Utils.matToBitmap(dest, result)
        src.release(); gray.release(); denoised.release(); dest.release()
        return result
    }
    ...
    fun normalize(bitmap: Bitmap): Bitmap {
        ...
        val result = Bitmap.createBitmap(bitmap.width, bitmap.height, Bitmap.Config.ARGB_8888)
        Utils.matToBitmap(dest, result)
        src.release(); gray.release(); normalized.release(); dest.release()
        return result
    }
